    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(update_query, (
                str(uuid.uuid4()), project_id, project_id, 
                json.dumps(chat_history), datetime.now(), datetime.now()
            ))
            conn.commit()
...
def update_project_status(project_id, new_status):
    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(update_query, (new_status, project_id))
            conn.commit()
    except Exception as e:
...
def update_project_kpi_data(project_id, kpi_data):
    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(update_query, (json.dumps(kpi_data), project_id))
            conn.commit()
    except Exception as e:
...
"""
        ...
        try:
            ...
            if sentiment == "like":
                ...
                """
                with engine.connect() as conn:
                    conn.execute(save_query, (project_id, entity_id, datetime.now()))
                    conn.commit()
            ...
            with engine.connect() as conn:
                conn.execute(sentiment_query, (
                    str(uuid.uuid4()), project_id, project_id, entity_id, sentiment, datetime.now()
                ))
                conn.commit()
...
"""
        ...
        with engine.connect() as conn:
            conn.execute(insert_query, (
                template_id, project_id, project_data['user_id'], target_entity_id, 
                platform, json.dumps([]), template_content, datetime.now(), datetime.now()
            ))
            conn.commit()
